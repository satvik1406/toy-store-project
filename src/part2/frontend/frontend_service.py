...
class CustomHandler(BaseHTTPRequestHandler):
    ...
    def read_term_from_file():
        """ Get the TERM value from the disk """
        global TERM
        try:
            with open(TERM_FILE, 'r') as file:
                TERM = int(file.read().strip())
                print(f"The current term as {TERM} from file")
        except FileNotFoundError:
            print(f"No term file found, starting with term {TERM}")

    def write_term_to_file():
        """ Write the TERM value to the disk """
        with open(TERM_FILE, 'w') as file:
            file.write(str(TERM))
            print(f"Current term {TERM} written to file")
    ...
    def do_POST(self):
        # Check if the path starts with '/orders' indicating an order request.
    ...
    def elect_leader():
        ...
        for priority, port in sorted_replicas:
            ...
            if CustomHandler.is_port_open(ORDER_HOST, port):
                ...
                # We increment the TERM when a new leader is elected and written to the disk
                TERM = TERM + 1
                CustomHandler.write_term_to_file()
                break
        return leader
...
if __name__ == "__main__":
    ...
    CustomHandler.read_term_from_file()
    leader = CustomHandler.elect_leader()
    if leader != 0:
        ORDER_PORT, ORDER_NODE= leader[1], leader[0]
